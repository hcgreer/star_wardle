...
const targetCharacter = {"name": "Greedo", "gender": "male", "homeworld": "Rodia", "species": "Rodian"};
let currentGuesses = [];
...
function submitGuess() {
    guessForm.addEventListener("submit", (e) => {
        ...
        const guessCharacter = characters.find((p) => {
            return p.name.toLowerCase() === guessText.toLowerCase() && !currentGuesses.includes(p.name);
        })
        if (guessCharacter) {
            storeGuesses(guessCharacter.name);
            revealHeader();
        ...
        else if (currentGuesses.some(guess => guess.toLowerCase() === guessText.toLowerCase())){
            showAlert("Already guessed character.")
        }
        else{
...
guessInput.onkeyup = function() {
    ...
    if (input.length){
        resultList = characters.filter((person) => {
            return person.name.toLowerCase().startsWith(input.toLowerCase()) && !currentGuesses.includes(person.name);
        });
    }
